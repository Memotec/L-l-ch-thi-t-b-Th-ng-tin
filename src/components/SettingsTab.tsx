          ...
          <div className="lg:col-span-2 enterprise-card p-5 space-y-5">
            ...
            <div className="p-4 bg-indigo-50 border border-indigo-200 rounded-lg space-y-3">
              ...
              <p className="text-[11px] text-indigo-900/80 leading-relaxed">
                Tự động tải về và ghi đè dữ liệu cục bộ từ <b>tệp JSON cơ sở dữ liệu hợp nhất duy nhất</b> được sao lưu trên máy chủ Cloud. Sử dụng tính năng này để khôi phục nhanh hồ sơ khi Firebase Firestore bị gián đoạn, mất kết nối hoặc vượt hạn ngạch (quota) sử dụng.
              </p>
              ...
              <div className="grid grid-cols-1 sm:grid-cols-2 gap-3 pt-1">
                  ...
                  <button
                    onClick={async () => {
                      if (window.confirm('CẢNH BÁO: Hành động này sẽ TẢI VỀ và GHI ĐÈ toàn bộ cơ sở dữ liệu hiện tại bằng tệp lưu trữ gốc từ Server! Bạn có chắc chắn muốn khôi phục?')) {
                        if (onRestoreFromCloudJson) {
                          await onRestoreFromCloudJson();
                        }
                      }
                    }}
                    className="flex items-center justify-center gap-2 py-2 px-3 bg-indigo-600 hover:bg-indigo-700 text-white rounded-lg text-xs font-bold transition-all shadow-sm cursor-pointer"
                    title="Tải tệp JSON từ Server và nạp đè dữ liệu hiện tại của hệ thống"
                  >
                    <RefreshCw className="w-3.5 h-3.5" />
                    <span>Nạp Dữ Liệu Từ Server JSON</span>
                  </button>
                ) : (
                  <button
                    onClick={onOpenLoginModal}
                ...
                <button
                  onClick={() => {
                    cloudSyncService.downloadUnifiedJsonDb();
                    onShowToast?.('✓ Đang chuẩn bị tải về file JSON cơ sở dữ liệu hợp nhất từ máy chủ...');
                  }}
                  className="flex items-center justify-center gap-2 py-2 px-3 bg-white hover:bg-indigo-50 text-indigo-900 border border-indigo-300 rounded-lg text-xs font-semibold transition-all shadow-xs cursor-pointer"
                  title="Tải trực tiếp tệp tin cns_unified_equipment_database.json từ Server"
                >
                  <Download className="w-3.5 h-3.5 text-indigo-600" />
                  <span>Tải File JSON Server Về Máy</span>
                </button>
              </div>
            </div>
